const app = new Vue({
  ...
  methods: {
    ...
    createMessageTable: function () {
      ...
      const clinicList = this.clinicList.filter((clinic) => {
        return clinic.city === vm.contact.address;
      });
      if (clinicList.length === 0) return "無符合條件的診所資料";
    ...
    saveContactInfo: function () {
      ...
      const scriptURL =
        "https://script.google.com/macros/s/AKfycbxy0GjG83Qqy2Wh0AwF3LkLQtoF7wOTe9gQ9KD4-lpCAFu9lM63gZtr9NrSEVtBOW-V/exec";
    ...
    sendMailApi: function () {
      ...
      const templateParams = {
        ...
        from_name: "Nourkrin 盈法關鍵補給錠-洛克靈", // 寄件人名字
        reply_to: "nourkrin.taiwan@gmail.com",
        subject: `感謝您的填寫 — ${vm.contact.address} NourKrin 認證診所資訊`,
        message: vm.createMessageTable(),
        name: vm.contact.name,
      };

      emailjs
        .send("service_6slzxkr", "template_1pjgjag", templateParams)
        .then(async (response) => {
          alert("填寫成功!");
  ...
  mounted: function () {
    const vm = this;
    emailjs.init({
      publicKey: "C57fevBpYSMBCqf11",
    });
    fetch("assets/nourkrin/clinic.json")
